...
export async function claimPaymentCompletedEmailLock(id) {
  const data = await supabaseAdminRequest(
    `${TABLE_NAME}?${new URLSearchParams({
      id: `eq.${id}`,
      payment_completed_email_sent_at: "is.null",
      payment_completed_email_lock_at: "is.null",
    }).toString()}`,
    {
      method: "PATCH",
      headers: {
      ...
      body: {
        payment_completed_email_lock_at: new Date().toISOString(),
      },
    },
  );
