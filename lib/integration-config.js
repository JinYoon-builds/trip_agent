...
export function isPayPalConfigured() {
  return Boolean(
    readOptionalEnv("PAYPAL_ENV") &&
      readOptionalEnv("PAYPAL_CLIENT_ID") &&
      readOptionalEnv("PAYPAL_CLIENT_SECRET") &&
      readOptionalEnv("NEXT_PUBLIC_PAYPAL_CLIENT_ID"),
  );
}

export function getPayPalServerConfig() {
  const env = readOptionalEnv("PAYPAL_ENV").toLowerCase();
  const clientId = readOptionalEnv("PAYPAL_CLIENT_ID");
  const clientSecret = readOptionalEnv("PAYPAL_CLIENT_SECRET");
  const publicClientId = readOptionalEnv("NEXT_PUBLIC_PAYPAL_CLIENT_ID");
  const missingKeys = [];

  if (env !== "sandbox" && env !== "live") {
    missingKeys.push("PAYPAL_ENV (sandbox or live)");
  }
  ...
  if (!publicClientId) {
    missingKeys.push("NEXT_PUBLIC_PAYPAL_CLIENT_ID");
  }
