...
export async function createPayPalOrderForSubmission(submissionId) {
  const headers = await createAuthHeaders({
    "Content-Type": "application/json",
  });
  const response = await fetch(`/api/submissions/${submissionId}/paypal/order`, {
    method: "POST",
    headers,
    body: JSON.stringify({}),
  });
...
export async function capturePayPalOrderForSubmission(submissionId, orderId) {
  const headers = await createAuthHeaders({
    "Content-Type": "application/json",
  });
  const response = await fetch(`/api/submissions/${submissionId}/paypal/capture`, {
    method: "POST",
    headers,
